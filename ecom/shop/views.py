...
def collection(request):
    ...
    # Get selected gender from GET params
    selected_gender = request.GET.get('gender', '')
    
    # Filter by gender if present
    if selected_gender:
        products = products.filter(gender__iexact=selected_gender)

    # Apply the rest of the filters (like category, price, name)
    ...
    return render(request, 'collection.html', {
        ...
        'selected_gender': selected_gender,  # pass to template for button highlighting
    })
...
def weborders(request):
    return render(request, 'weborders.html')